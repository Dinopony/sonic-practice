...
Code& Code::addq(uint8_t value, const Param& param, Size size)
{
    uint16_t size_code = 0x0;
    if (size == Size::WORD)
        size_code = 0x1;
    ...
    uint16_t opcode = 0x5000 + ((uint16_t)(value & 0x7) << 9) + (size_code << 6) + param.getMXn();
    this->add_opcode(opcode);
    this->add_bytes(param.getAdditionnalData());
    return *this;
}

Code& Code::subq(uint8_t value, const Param& param, Size size)
{
    uint16_t size_code = 0x0;
    if (size == Size::WORD)
        size_code = 0x1;
    ...
    uint16_t opcode = 0x5100 + ((uint16_t)(value & 0x7) << 9) + (size_code << 6) + param.getMXn();
    this->add_opcode(opcode);
    this->add_bytes(param.getAdditionnalData());
    return *this;
}
...
Code& Code::move_to_sr(uint16_t value)
{
    this->add_opcode(0x46FC);
    this->add_word(value);
    return *this;
}
...
const std::vector<uint8_t>& Code::get_bytes() const
{
    ...
    {
        std::string msg = std::string("Pending branch is unresolved on injected code : ") + _pending_branches.begin()->second;
...
void Code::resolve_branches()
{
    ...
    for (auto& [branch_address, label] : _pending_branches)
    {
        if (_labels.count(label))
        {
            ...
            {
                throw std::out_of_range("Offset for branch at byte " + std::to_string(branch_address) +
